/**
 * Amplify - Service Worker (Chromium MV3)
 * Handles communication between popup and content scripts
 */

console.log('Amplify: Service worker loaded');
...
async function setTabVolume(tabId, volume) {
  ...
  await chrome.storage.local.set({ volume: volume });
...
async function getTabVolume(tabId) {
  ...
  try {
    const response = await chrome.tabs.sendMessage(tabId, { action: 'getVolume' });
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  console.log('Amplify: Service worker received', message);
...
console.log('Amplify: Service worker ready');
